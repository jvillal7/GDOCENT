...
export async function proposarCoberturaOriol(absentNom, frangesIds, docents, normes, data, infoExtra, baixes, frangesIA = null) {
  const dia = data
  ...
  const FRANJES_ACT = frangesIA || FRANJES_ORIOL;
  ...
  for (const [nom, info] of Object.entries(assignacioMap)) {
    ...
    const firstF = FRANJES_ACT.find(f => f.id === sorted[0]);
    const lastF  = FRANJES_ACT.find(f => f.id === sorted[sorted.length - 1]);
  ...
  const assignacioLines = Object.entries(assignacioMap).map(([nom, info]) => {
    const hores = info.fids.map(fid => FRANJES_ACT.find(x => x.id === fid)?.sub || fid).join(' / ');
...
• PAE REFERENT SOL: permès com a últim recurs, però mai com a primera opció.
...
2. Aplica la jerarquia de les NORMES DEL CENTRE. Para en el primer nivell possible.
3. Si el mateix docent cobreix franges consecutives → una sola entrada amb totes les franges_ids.
