...
async function proposarCoberturaOriol(absentNom, frangesIds, docents, normes, data, infoExtra, baixes) {
  const dia = data
    ? ['diumenge','dilluns','dimarts','dimecres','dijous','divendres','dissabte'][new Date(data + 'T12:00:00').getDay()]
    : null;
  ...
  // Pre-assignment: MEE already with absent group
  const assignacioMap = {};
  const assignedFids = new Set();
  if (dia && absentGrupN) {
    for (const fid of frangesIds) {
  ...
  // Build availability summary per docent
  const disponibilitatDocents = docents
    .filter(d => d.horari && d.nom !== absentNom)
    .map(d => {
...
async function callClaudeRaw(messages, maxTokens = 2000) {
  const res = await fetch(WORKER_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Auth-Token': WORKER_AUTH_TOKEN },
    body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: maxTokens, messages }),
