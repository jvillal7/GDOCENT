...
export function makeApi(escolaId) {
  ...
  return {
    ...
    saveContextIA:       txt   => f(`escoles?id=eq.${escolaId}`, { method: 'PATCH', body: JSON.stringify({ context_ia: txt }), bypassSchoolId: true }),
    getFrangesIA:        ()    => f(`escoles?id=eq.${escolaId}&select=franges_ia`, { bypassSchoolId: true }),
    ...
    getOriolDiari:       ()    => f(`escoles?id=eq.${escolaId}&select=oriol_absents,oriol_reunions,oriol_ceepsir,oriol_baixes,oriol_pdf_data,oriol_lema`, { bypassSchoolId: true }),
