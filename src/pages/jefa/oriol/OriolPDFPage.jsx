...
function buildTaulaEspecialistes(cobertures, docents, todayDia, absentsAvui) {
  ...
  return cobrintNoms.map((nom, idx) => {
    ...
    function labelSlot(fid, rawVal) {
      ...
      // Pati — detectar per valor explícit; per "Pati" sol usar l'ID de franja
      ...
      if (vl === 'pati' || vl === 'patio' || vl === 'descans') {
        if (fid === 'opatiA') return 'PATI PRIMÀRIA';
        if (fid === 'opatiB') return 'PATI SECUNDÀRIA';
        return 'PATI';
      }
