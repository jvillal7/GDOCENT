...
function getJwt() {
  try { return sessionStorage.getItem('gd_jwt') || null; } catch { return null; }
}
      ...
      <div style={{
        ...
        {teCorreccions && (
          <div style={{ padding: '8px 16px', background: '#fffbeb', borderBottom: '1px solid #fde68a', fontSize: 11, color: '#92400e' }}>
            ⚠️ La cap d'estudis ha fet {userMsgs.length - 1} correcció{userMsgs.length > 2 ? 's' : ''} — revisa les Normes apreses
          </div>
        )}
...
export default function LogsPage() {
  ...
  const logsFiltrats = filtre === 'tots' ? logs : logs.filter(l => l.resultat === filtre);

  const logsAmbCorreccions = logs.filter(l => (l.missatges || []).filter(m => m.role === 'user').length > 1);
    ...
    <div className="page-hdr" style={{ maxWidth: 800 }}>
      ...
      {logsAmbCorreccions.length > 0 && (
        <div style={{ padding: '10px 14px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, marginBottom: 16, fontSize: 12, color: '#92400e' }}>
          ⚠️ <strong>{logsAmbCorreccions.length}</strong> conversa{logsAmbCorreccions.length > 1 ? 's' : ''} amb correccions — revisa <strong>Normes IA → Normes apreses</strong> per confirmar les regles generades.
        </div>
      )}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 13 }}>
                    ...
                    {teCorreccions && (
                      <span style={{ fontSize: 11, marginLeft: 6, color: '#92400e', fontWeight: 400 }}>
                        ({userMsgs.length - 1} correcció{userMsgs.length > 2 ? 's' : ''})
                      </span>
                    )}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 2 }}>
